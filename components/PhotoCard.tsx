...
import { Download, Play, RotateCcw, ImageIcon } from 'lucide-react'
...
function StatusBadge({ status }: { status: Job['status'] }) {
  switch (status) {
    case 'pending':
      return (
        <Badge className="bg-gray-700 text-gray-300 border-0">Aguardando</Badge>
      )
    case 'enhancing':
      return (
  ...
  return (
    <Card className="bg-gray-900 border-gray-800 overflow-hidden group">
      ...
      <div className="relative aspect-video bg-gray-800 overflow-hidden">
        ...
        {(job.status === 'enhancing' || job.status === 'decluttering') && (
      ...
      <CardContent className="p-3 space-y-3">
        ...
        <div className="flex items-start justify-between gap-2">
          <p
            className="text-sm text-gray-300 truncate flex-1 font-medium"
            title={displayName}
          >
            {displayName}
          </p>
